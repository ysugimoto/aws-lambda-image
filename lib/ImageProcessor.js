...
class ImageProcessor {
    ...
    processImage(imageData, config) {
        ...
        if ( config.exists("backup") ) {
            ...
            promise = promise.then(() => this.execBackupImage(backup, imageData).then(this.fileSystem.putObject));
            processedImages++;
        }
        ...
        if ( config.exists("reduce") ) {
            ...
            promise = promise.then(() => this.execReduceImage(reduce, imageData).then(this.fileSystem.putObject));
            processedImages++;
        }
        ...
        config.get("resizes", []).filter((resize) => {
            ...
            promise = promise.then(() => this.execResizeImage(resize, imageData).then(this.fileSystem.putObject));
            processedImages++;
        });
